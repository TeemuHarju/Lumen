...
b8 platform_startup( platform_state* plat_state, const char* application_name, i32 x, i32 y, i32 width, i32 height )
{
	...
	// Show the window.
	b32 should_activate = 1;  // TODO: if the window should not accept input, set this to 0.
	i32 show_window_command_flags = should_activate ? SW_SHOW : SW_SHOWNOACTIVATE;
	// if initially minimized, use SW_MINIMIZE : SW_SHOWMINNOACTIVE;
	// if initilaized maximized, use SW_SHOWMAXIMIZED : SW_MAXIMIZE;
...
LRESULT CALLBACK win32_process_messages( HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param )
{
	...
	case WM_SIZE:
	{
		...
		// TODO: Fire an event for the application to handle the window resize.
		break;
	}
	case WM_KEYDOWN:
	case WM_SYSKEYDOWN:
